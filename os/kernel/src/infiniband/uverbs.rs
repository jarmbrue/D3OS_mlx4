...
use crate::{
    device::mlx4::{device_in_range, ConnectX3Nic},
    security::syscall_sec::copy_from_user,
};
use alloc::vec;
...
pub fn uverbs_ctl(minor: usize, cmd: usize, arg: usize) -> SyscallResult {
    ...
    match cmd {
        UVERBS_CMD_QUERY_DEVICES => {
            ...
            copy_to_user(__user_buf, __kernel_buf.as_ptr().cast(), query_h * size_of::<usize>())?;
        ...
        UVERBS_CMD_CREATE_CQ => {
            ...
            copy_from_user(
                inline_cast!(mut &mut __kernel_cq_container, ibv_cq_container, u8),
                __user_buf.cast(),
                size.into(),
            )?;
        ...
        UVERBS_CMD_CREATE_QP => {
            ...
            copy_from_user(
                inline_cast!(mut &mut __kernel_qp_container, ibv_qp_container, u8),
                __user_buf.cast(),
                size.into(),
            )?;
            ...
            copy_from_user(
                inline_cast!(mut &mut __kernel_ib_caps, ibv_qp_cap, u8),
        ...
        UVERBS_CMD_MODIFY_QP => {
            ...
            copy_from_user(
                inline_cast!(mut &mut __kernel_qp_modify_container, ibv_qp_modify_container, u8),
                __user_buf.cast(),
                size.into(),
            )?;
        ...
        UVERBS_CMD_POLL_CQ => {
            ...
            copy_from_user(
                inline_cast!(mut &mut __kernel_cq_poll_container, ibv_cq_poll_container, u8),
                __user_buf.cast(),
                size.into(),
            )?;
