...
pub fn uverbs_ctl(minor: usize, cmd: UverbsCmd, user_memory_addr: *const UserMemory) -> SyscallResult {
    ...
    if requires_device_handle && !device_in_range(minor) {
        error!("{:?} requires a device handle, but {} is out of range", cmd, minor);
        return Err(Errno::EINVAL);
    }

    if !requires_device_handle && minor != 0 {
        error!("{:?} requires no device handle, but got {}", cmd, minor);
        return Err(Errno::EINVAL);
    }
