...
pub struct WorkCompletion {
    ...
    ///    Completions
    pub wc_flags: WorkCompletionFlags,
...
impl WorkCompletion {
    ...
    pub fn is_valid(&self) -> bool {
        self.status == WorkCompletionStatus::Success
    }
    ...
    pub fn error(&self) -> Option<(WorkCompletionStatus, u32)> {
        match self.status {
            WorkCompletionStatus::Success => None,
    ...
    pub fn opcode(&self) -> WorkCompletionOpcode {
        self.opcode
    }
    ...
    pub fn imm_data(&self) -> Option<u32> {
        if self.is_valid() && self.wc_flags.contains(WorkCompletionFlags::IBV_WC_WITH_IMM) {
            Some(self.imm_data)
        } else {
            None
        }
...
impl Default for WorkCompletion {
    fn default() -> Self {
        WorkCompletion {
            wr_id: 0,
            status: WorkCompletionStatus::GeneralError,
            opcode: WorkCompletionOpcode::LocalInvalidate,
            vendor_err: 0,
            byte_len: 0,
            imm_data: 0,
            qp_num: 0,
            src_qp: 0,
            wc_flags: WorkCompletionFlags::empty(),
            pkey_index: 0,
            slid: 0,
            sl: 0,
            dlid_path_bits: 0,
...
pub enum WorkCompletionStatus {
    ...
    RemoteAbortedErr,
    GeneralError,
    /* TODO: add work completion states representing following linux states:
...
bitflags! {
    ...
    pub struct WorkCompletionFlags: u32 {
        const IBV_WC_GRH = 1;
